...
func TestEvictWorkerPods_MarksEvictedPodLabel(t *testing.T) {
	p1 := newPod("p1")
	fakeClient := clientgofake.NewClientset(p1)
...
func TestCheckDefragPDBPreflight_BlockedPDBSkipsWholeNode(t *testing.T) {
	...
	fakeClient := clientgofake.NewClientset(p1, p2, pdb)
	r := newReconcilerWithFake(fakeClient)
	cand := &defragCandidate{
		nodeName:   testDefragNodeA,
...
func TestCheckDefragPDBPreflight_AllowsNodeWhenDisruptionsAvailable(t *testing.T) {
	...
	fakeClient := clientgofake.NewClientset(p1, pdb)
	r := newReconcilerWithFake(fakeClient)
	cand := &defragCandidate{
		nodeName:   testDefragNodeA,
		workerPods: []*corev1.Pod{p1},
...
func TestCheckDefragPDBPreflight_ListsPDBsOncePerNamespace(t *testing.T) {
	...
	directClient := clientgofake.NewClientset(p1, p2, pdb)
...
func TestEvictWorkerPods_HonorsCtxDeadline(t *testing.T) {
	fakeClient := clientgofake.NewClientset(newPod("p1"))
	r := newReconcilerWithFake(fakeClient)
...
func TestCollectDefragCandidates_SkipsFreshWorkerPods(t *testing.T) {
	...
	objects := []ctrlclient.Object{pool, node, gpuNode, freshWorker} //nolint:prealloc // verbatim from v1, kept terse for diff parity
...
func TestCollectDefragCandidates_IncludesOldWorkerPods(t *testing.T) {
	...
	objects := []ctrlclient.Object{pool, node, gpuNode, oldWorker} //nolint:prealloc // verbatim from v1, kept terse for diff parity
	objects = append(objects, newDefragNodeGPUs(testDefragNodeA)...)

	r, _ := newDefragControllerTestReconciler(t, objects...)
	r.KubeClient = clientgofake.NewClientset(newDefragWorkerPDB())
	if err := r.Allocator.InitGPUAndQuotaStore(); err != nil {
		t.Fatalf("init GPU store: %v", err)
	}
	r.Allocator.ReconcileAllocationStateForTesting()
...
func TestCollectDefragCandidates_SkipsNodeWhenPDBSelectorIsEmpty(t *testing.T) {
	...
	r, _ := newDefragControllerTestReconciler(t, objects...)
	r.KubeClient = clientgofake.NewClientset(emptySelectorPDB)
	if err := r.Allocator.InitGPUAndQuotaStore(); err != nil {
		t.Fatalf("init GPU store: %v", err)
	}
	r.Allocator.ReconcileAllocationStateForTesting()
...
func TestCollectDefragCandidates_SkipsNodeWithNativeGPUPod(t *testing.T) {
	...
	objects := []ctrlclient.Object{pool, node, gpuNode, oldWorker, nativePod} //nolint:prealloc // verbatim from v1, kept terse for diff parity
	objects = append(objects, newDefragNodeGPUs(testDefragNodeA)...)
...
func TestCollectDefragCandidates_SkipsNodeWithAMDGPUPod(t *testing.T) {
	...
	objects := []ctrlclient.Object{pool, node, gpuNode, oldWorker, amdPod} //nolint:prealloc // verbatim from v1, kept terse for diff parity
	objects = append(objects, newDefragNodeGPUs(testDefragNodeA)...)
...
func TestCollectDefragCandidates_AllowsNodeWithNonGPUSidecarPod(t *testing.T) {
	...
	objects := []ctrlclient.Object{pool, node, gpuNode, oldWorker, sidecar} //nolint:prealloc // verbatim from v1, kept terse for diff parity
	objects = append(objects, newDefragNodeGPUs(testDefragNodeA)...)
...
func TestCollectDefragCandidates_AllowsNodeWhenOnlyTFWorkerOwnsGPURequest(t *testing.T) {
	...
	objects := []ctrlclient.Object{pool, node, gpuNode, worker} //nolint:prealloc // verbatim from v1, kept terse for diff parity
	objects = append(objects, newDefragNodeGPUs(testDefragNodeA)...)
...
func TestCollectDefragCandidates_AllowsNodeWhenNativeGPUPodIsTerminating(t *testing.T) {
	...
	objects := []ctrlclient.Object{pool, node, gpuNode, oldWorker, terminating} //nolint:prealloc // verbatim from v1, kept terse for diff parity
	objects = append(objects, newDefragNodeGPUs(testDefragNodeA)...)
...
func TestSafetySweepAndGuards_BlocksOnLingeringSourceNode(t *testing.T) {
	...
	objects := []ctrlclient.Object{ //nolint:prealloc // verbatim from v1, kept terse for diff parity
...
func TestCollectDefragCandidates_SkipsEvictSkipNode(t *testing.T) {
	...
	moreObjects := []ctrlclient.Object{otherEvictSkipNode, otherGPUNode, otherWorker} //nolint:prealloc // verbatim from v1, kept terse for diff parity
	...
	r2.KubeClient = clientgofake.NewClientset(newDefragWorkerPDB())
...
func newDefragControllerTestReconciler(
	...
	allocator := gpuallocator.NewGpuAllocator(context.Background(), nil, kubeClient, time.Minute)
...
// PDB-covered.
func newDefragWorkerPDB() *policyv1.PodDisruptionBudget {
...
func assertWarningEvent(t *testing.T, recorder events.EventRecorder, reason, substr string) {
	t.Helper()
	fr, ok := recorder.(*events.FakeRecorder)
	if !ok {
		t.Fatalf("recorder is %T, want *events.FakeRecorder", recorder)
	}
	for {
		select {
		case ev := <-fr.Events:
...
func assertNoEvent(t *testing.T, recorder events.EventRecorder, reason string) {
	t.Helper()
	fr, ok := recorder.(*events.FakeRecorder)
	if !ok {
		t.Fatalf("recorder is %T, want *events.FakeRecorder", recorder)
...
// TestTargetAcceptsAnotherWorker pins the at-cap boundary: a node whose
// current worker count equals maxWorkerPerNode must be rejected as a
// defrag target, because the real allocator's Assume() rejects on
// `>=`. Using the prior `>` comparison let defrag virtually place onto
// a full node, then real reschedule failed and left source/evicted
// markers behind.
func TestTargetAcceptsAnotherWorker(t *testing.T) {
	cases := []struct {
		name             string
		workerCount      int
		...
		{"no cap configured (max=0) always accepts", 5, 0, true},
		{"negative cap treated as no cap", 5, -1, true},
		{"below cap accepts", 1, 2, true},
		{"at cap rejects (the bug)", 2, 2, false},
		{"over cap rejects", 3, 2, false},
		{"zero workers always accepts", 0, 1, true},
	}
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			nb := &nodeBudget{workerCount: c.workerCount}
			if got := targetAcceptsAnotherWorker(nb, c.maxWorkerPerNode); got != c.want {
				t.Fatalf("workerCount=%d max=%d: got=%v want=%v",
					c.workerCount, c.maxWorkerPerNode, got, c.want)
			}
		})
	}
	t.Run("nil budget rejects defensively", func(t *testing.T) {
		if got := targetAcceptsAnotherWorker(nil, 2); got != false {
			t.Fatalf("nil budget: got=%v want=false", got)
		}
	})
}

// TestCleanupPerPoolState pins that pool deletion drains every per-pool
// entry from the three sync.Maps that defrag/compaction keep keyed by
// pool name. Pre-fix Reconcile only stopped writing to these maps for a
// deleted pool, so each one would slowly accumulate across pool churn.
func TestCleanupPerPoolState(t *testing.T) {
	const poolName = "test-cleanup-pool-xyz"
	req := ctrl.Request{NamespacedName: types.NamespacedName{Name: poolName}}

	jobStarted.Store(req.String(), time.Now())
	defragLastRunStats.Store(poolName, &defragRunStats{StartTime: time.Now()})
	r := &GPUPoolCompactionReconciler{}
	r.defragRunning.Store(poolName, &atomic.Bool{})
	...
	for _, present := range []struct {
		...
		{"jobStarted", func() bool { _, ok := jobStarted.Load(req.String()); return ok }()},
		{"defragLastRunStats", func() bool { _, ok := defragLastRunStats.Load(poolName); return ok }()},
		{"defragRunning", func() bool { _, ok := r.defragRunning.Load(poolName); return ok }()},
	} {
		if !present.ok {
			t.Fatalf("setup: %s missing before cleanup", present.name)
		}
	}

	r.cleanupPerPoolState(req)

	if _, ok := jobStarted.Load(req.String()); ok {
		t.Fatal("jobStarted not cleared after cleanupPerPoolState")
	}
	if _, ok := defragLastRunStats.Load(poolName); ok {
		t.Fatal("defragLastRunStats not cleared after cleanupPerPoolState")
	}
	if _, ok := r.defragRunning.Load(poolName); ok {
		t.Fatal("defragRunning not cleared after cleanupPerPoolState")
	}
}

// TestReconcile_IntervalGate_StillAdvancesDefrag pins the rapid-requeue
// path that the port originally broke: when jobStarted shows a recent
// compaction so the interval gate sets !needStartCompactionJob, Reconcile
// must still dispatch defrag so an active campaign can drain promptly
// (e.g., return defragActiveStepRequeue=10s). The pre-fix Reconcile
// returned ctrl.Result{RequeueAfter: nextDuration} without ever calling
// the defrag step, pinning campaigns behind a full compaction period.
func TestReconcile_IntervalGate_StillAdvancesDefrag(t *testing.T) {
	const poolName = "test-interval-gate-defrag"
	req := ctrl.Request{NamespacedName: types.NamespacedName{Name: poolName}}
	// Tests share package-level maps; isolate this test's keys.
	t.Cleanup(func() {
		jobStarted.Delete(req.String())
		defragLastRunStats.Delete(poolName)
	})
	...
	pool := &tfv1.GPUPool{
		...
		Spec: tfv1.GPUPoolSpec{
			NodeManagerConfig: &tfv1.NodeManagerConfig{
				NodeCompaction: &tfv1.NodeCompaction{
					Period: "5m",
					Defrag: &tfv1.NodeDefragConfig{Enabled: true},
				},
			},
		},
	}
	...
	// Interval gate: pretend compaction just ran. The pre-fix Reconcile
	// returns immediately here without calling defrag.
	jobStarted.Store(req.String(), time.Now())
	t.Cleanup(func() { jobStarted.Delete(req.String()) })

	// Stub defragStepFn so we observe the dispatch without standing up a
	// real scheduler. A 10s return mirrors defragActiveStepRequeue, the
	// value the real guard path returns when an evicted-pod marker is
	// still in flight.
	var defragCalls atomic.Int32
	r.defragStepFn = func(_ context.Context, p *tfv1.GPUPool, normal time.Duration) time.Duration {
		defragCalls.Add(1)
		if p.Name != poolName {
			t.Errorf("defrag dispatched with wrong pool: got %q want %q", p.Name, poolName)
		}
	...
	if got := defragCalls.Load(); got != 1 {
		t.Fatalf("expected defrag dispatched exactly once under interval gate, got %d", got)
	}
	if result.RequeueAfter != 10*time.Second {
		t.Fatalf("expected RequeueAfter=10s from defrag, got %s", result.RequeueAfter)
	}
}

// TestReconcile_IntervalGate_SkipsDefragOnPendingClaim pins the other
// half of the same branch: a pending GPUNodeClaim means new capacity is
// being added, so draining during expansion is wasted churn. Defrag must
// stay quiet, and Reconcile must return the normal interval requeue.
func TestReconcile_IntervalGate_SkipsDefragOnPendingClaim(t *testing.T) {
	const poolName = "test-interval-gate-pending-claim"
	req := ctrl.Request{NamespacedName: types.NamespacedName{Name: poolName}}
	t.Cleanup(func() { jobStarted.Delete(req.String()) })
	...
	// Mark a pending GPUNodeClaim for this pool (any non-empty entry
	// makes hasPendingClaim true).
	pendingGPUNodeStateLock.Lock()
	PendingGPUNodeClaim[poolName] = map[string]tfv1.Resource{"claim-1": {}}
	pendingGPUNodeStateLock.Unlock()
	t.Cleanup(func() {
		pendingGPUNodeStateLock.Lock()
		delete(PendingGPUNodeClaim, poolName)
		pendingGPUNodeStateLock.Unlock()
	})

	var defragCalls atomic.Int32
	r.defragStepFn = func(_ context.Context, _ *tfv1.GPUPool, _ time.Duration) time.Duration {
		defragCalls.Add(1)
		return 10 * time.Second
	}
	...
	if got := defragCalls.Load(); got != 0 {
		t.Fatalf("defrag must not run while a GPUNodeClaim is pending, got %d calls", got)
	}
	// Period parses to 5m; this is the only thing returned when defrag stays quiet.
	if result.RequeueAfter != 5*time.Minute {
		t.Fatalf("expected RequeueAfter=5m (NodeCompaction.Period), got %s", result.RequeueAfter)
	}
}
...
func newSharedNodeWorkloadFixture(t *testing.T) (*GPUPoolCompactionReconciler, ctrlclient.Client, *tfv1.GPUPool, *corev1.Pod, *corev1.Pod) {
	...
	objects := append(newDefragNodeGPUs(testDefragNodeA), pool, node, gpuNode, podA, podB)
	r, kubeClient := newDefragControllerTestReconciler(t, objects...)
	primeAllocatorWorkerStore(t, r)
	return r, kubeClient, pool, podA, podB
}

func newDefragSourceNode(poolName string) *corev1.Node {
	return &corev1.Node{ObjectMeta: metav1.ObjectMeta{
		Name: testDefragNodeA,
		UID:  types.UID(testDefragNodeA + "-uid"),
		Labels: map[string]string{
			constants.DefragSourceNodeLabel: constants.TrueStringValue,
		},
		Annotations: map[string]string{
...
func getDefragTestNode(t *testing.T, kubeClient ctrlclient.Client) *corev1.Node {
	t.Helper()
	node := &corev1.Node{}
	if err := kubeClient.Get(context.Background(), types.NamespacedName{Name: testDefragNodeA}, node); err != nil {
		t.Fatalf("get node %s: %v", testDefragNodeA, err)
	}
	return node
}
...
func TestEvictWorkerPods_TooManyRequestsKeepsSourceMarker(t *testing.T) {
	...
	node := getDefragTestNode(t, kubeClient)
...
func TestContinueActiveDefragSourceNode_WaitsForInFlightEviction(t *testing.T) {
	now := time.Now()
	pool := newDefragTestPool()
	node := newDefragSourceNode(pool.Name)
...
func TestContinueActiveDefragSourceNode_EvictsNextPod(t *testing.T) {
	...
	gpuNode := newDefragGPUNode(testDefragNodeA)
	podB := newDefragWorkerPod("worker-b", testDefragNodeA, now.Add(-2*time.Hour))
	objects := append(newDefragNodeGPUs(testDefragNodeA), pool, node, gpuNode, podB)
	r, _ := newDefragControllerTestReconciler(t, objects...)
	r.Scheduler = &k8sscheduler.Scheduler{}
	primeAllocatorWorkerStore(t, r)
	r.defragSimulate = func(context.Context, *tfv1.GPUPool, *defragCandidate, int) (bool, *defragPlacementDiagnostics, error) {
