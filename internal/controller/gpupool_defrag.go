...
func (r *GPUPoolCompactionReconciler) placeSinglePod(
	...
	for _, info := range feasible {
		...
		if defragRejectsMaxWorkerPerNode(nb, maxWorkerPerNode, tgt, diag) {
			continue
		}
...
func (d *defragPlacementDiagnostics) summary() string {
	...
	rejects := make([]string, 0, minInt(4, len(pd.Rejections)))
	...
	if len(rejects) > 0 {
		...
		parts = append(parts, "scheduler="+strings.Join(pd.SchedulerDetails[:minInt(2, len(pd.SchedulerDetails))], ","))
	} else {
...
func (d *defragPlacementDiagnostics) budgetAccountingSummary() string {
	...
	out := make([]string, 0, minInt(4, len(d.BudgetNodes)))
...
func (d *defragPlacementDiagnostics) exclusionSummary() string {
	...
	out := make([]string, 0, minInt(4, len(d.ExcludedNodes)))
...
func defragRejectsMaxWorkerPerNode(nb *nodeBudget, maxWorkerPerNode int, target string, diag *defragPlacementDiagnostics) bool {
	if maxWorkerPerNode <= 0 || nb == nil || nb.workerCount < maxWorkerPerNode {
		return false
	}
	if diag != nil {
		diag.reject(target, "max-worker-per-node", fmt.Sprintf("workerCount=%d max=%d", nb.workerCount, maxWorkerPerNode))
	}
	return true
}
