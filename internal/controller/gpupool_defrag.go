...
const (
	...
	// Keep reconciles frequent inside an active campaign window when there is
	// nothing to evict yet, so MaxDuration is not shadowed by a long compaction period.
...
func (r *GPUPoolCompactionReconciler) maybeRunDefragStep(ctx context.Context, pool *tfv1.GPUPool, normalRequeue time.Duration) time.Duration {
	...
	if err != nil {
		...
		r.Recorder.Eventf(pool, nil, corev1.EventTypeWarning, defragEventConfigInvalid, defragEventConfigInvalid,
	...
	if guard.blockedByEvictedPod {
		...
		r.Recorder.Eventf(pool, nil, corev1.EventTypeNormal, defragEventBlockedEvictedPod, defragEventBlockedEvictedPod,
	...
	if guard.blockedBySourceNode {
		...
		r.Recorder.Eventf(pool, nil, corev1.EventTypeNormal, defragEventBlockedSourceNode, defragEventBlockedSourceNode,
...
func defragRequeueAfter(result defragStepResult, normalRequeue time.Duration) time.Duration {
	...
	if result.evictedNode || result.abortedNode || result.blockedByEvictedPod || result.blockedBySourceNode {
		if defragActiveStepRequeue < normalRequeue {
			return defragActiveStepRequeue
		}
	}
	if result.idleStep && result.finishReason != defragFinishReasonDeadline {
...
func (r *GPUPoolCompactionReconciler) runDefragStep(
	...
	announceStart := func() {
		...
		r.Recorder.Eventf(pool, nil, corev1.EventTypeNormal, defragEventStarted, defragEventStarted,
	...
	defer func() {
		...
		r.Recorder.Eventf(pool, nil, corev1.EventTypeNormal, defragEventFinished, defragEventFinished,
...
func (r *GPUPoolCompactionReconciler) processDefragCandidate(
	...
	if pdbBlocked {
		...
		r.Recorder.Eventf(pool, nil, corev1.EventTypeNormal, defragEventSkipPDBBlocked, defragEventSkipPDBBlocked,
...
// (gpuallocator.go:1086 / :1271): allocator rejects when
// workerCounts >= max, so a node already AT cap cannot accept a new
// worker. Using `>=` here (not `>`) keeps the simulation honest:
// otherwise defrag would virtually place onto a full node, then real
// Assume() rejects, leaving stale source/evicted markers behind.
// maxWorkerPerNode <= 0 means "no cap configured".
func targetAcceptsAnotherWorker(nb *nodeBudget, maxWorkerPerNode int) bool {
	if nb == nil {
		return false
	}
	if maxWorkerPerNode <= 0 {
		return true
	}
...
func (r *GPUPoolCompactionReconciler) evictWorkerPods(
	...
	if err := r.markNodeDefragSource(ctx, pool.Name, cand.nodeName); err != nil {
		stats.EvictionFailures++
		r.Recorder.Eventf(pool, nil, corev1.EventTypeWarning, defragEventPodEvictFailed, defragEventPodEvictFailed,
	...
	for _, pod := range cand.workerPods {
		...
		if err == nil {
			...
			stats.EvictedPods++
			r.Recorder.Eventf(pool, nil, corev1.EventTypeNormal, defragEventPodEvicted, defragEventPodEvicted,
		...
		if skipErr := r.markNodeDefragEvictSkip(ctx, pool.Name, cand.nodeName, reason); skipErr != nil {
			...
			r.Recorder.Eventf(pool, nil, corev1.EventTypeWarning, defragEventEvictSkip, defragEventEvictSkip,
