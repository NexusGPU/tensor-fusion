...
static void test_partition_templates(void) {
    printf("\n=== Partition templates ===\n");
    PartitionTemplate templates[8] = {0};
    size_t tmplCount = 0;
    Result r = GetPartitionTemplates(0, templates, 8, &tmplCount);
    TEST_ASSERT(r == RESULT_SUCCESS, "GetPartitionTemplates succeeds");
    TEST_ASSERT(tmplCount > 0, "Template count > 0");
    TEST_ASSERT(strlen(templates[0].templateId) > 0, "Template has ID");
}

static void test_device_topology(void) {
    printf("\n=== Device topology ===\n");
    int32_t indices[] = {0, 1};
    RelatedDevice connBuf[2][2];
    DeviceTopology topoDevices[2];
    memset(topoDevices, 0, sizeof(topoDevices));
    topoDevices[0].connections = connBuf[0];
    topoDevices[1].connections = connBuf[1];
    ...
    TEST_ASSERT(topo.devices[0].connectionCount <= 2, "Connection count within bound");
}
...
static void test_partition_lifecycle(void) {
    ...
    TEST_ASSERT(assignment.partitionOverheadBytes > 0, "Partition overhead set");
...
static void test_process_metrics(void) {
    ...
    size_t cuCount = 0;
    Result r = GetProcessComputeUtilization(cu, 4, &cuCount);
...
static void test_device_metrics(void) {
    ...
    DeviceMetrics dm[1];
    Result r = GetDeviceMetrics(deviceUUIDs, 1, dm);
    TEST_ASSERT(r == RESULT_SUCCESS, "GetDeviceMetrics succeeds");
    TEST_ASSERT(strlen(dm[0].deviceUUID) > 0, "Metrics device UUID set");

    uint32_t nvlinkBw[2] = {0};
    uint64_t ibBw[2] = {0};
    uint32_t pcieBw[2] = {0};
    ExtendedDeviceMetrics edm[1];
    memset(edm, 0, sizeof(edm));
    edm[0].nvlinkBandwidthMBps = nvlinkBw;
    edm[0].ibNicBandwidthMBps = ibBw;
    edm[0].pcieBandwidthMBps = pcieBw;
    r = GetExtendedDeviceMetrics(deviceUUIDs, 1, edm, 2, 2, 2);
    TEST_ASSERT(r == RESULT_SUCCESS, "GetExtendedDeviceMetrics succeeds");
    TEST_ASSERT(edm[0].pcieLinkCount > 0, "PCIe link count populated");
}
